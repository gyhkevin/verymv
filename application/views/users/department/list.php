...
<div class="am-cf admin-main">
	...
	<div class="am-g">
		...
		<table class="am-table am-table-bd am-table-striped admin-content-table">
			<thead>
			<tr>
			<th>ID</th><th>部门名称</th><th>部门全称</th><th>管理</th>
			</tr>
			</thead>
			<tbody>
			<?php foreach ($gotdata as $key => $val): ?>
				<tr><td><?=$val->id; ?></td><td><a href='<?=site_url("{$page_path}/edit/{$val->id}"); ?>' target='pandora_body'><?=$val->name; ?></a></td><td><?=$val->string; ?></td>
				<td>
					<div class="am-dropdown" data-am-dropdown>
					<button class="am-btn am-btn-default am-btn-xs am-dropdown-toggle" data-am-dropdown-toggle><span class="am-icon-cog"></span> <span class="am-icon-caret-down"></span></button>
					<ul class="am-dropdown-content">
						<li><a href='<?=site_url("{$page_path}/edit/{$val->id}"); ?>' target='pandora_body'>编辑</a></li>
						<li><a href='<?=site_url("{$page_path}/del_reco/{$val->id}"); ?>'>删除</a></li>
					</ul>
					</div>
				</td>
				</tr>
